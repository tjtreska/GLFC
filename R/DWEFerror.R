...
DWEFerror <- function(Dir, Catch, Lengths, Continue, Source = NULL) {
  ...
  if (length(unique(Catch$period)) != 1) {
    stop(
      "There should be just one value",
      " for the variable period in the Catch file."
    )
  }
  WHEN <- recode(Catch$period[1], c(0, 1, -1, NA), c(
    "No treatment",
  ...
  para(
    "The figures and tables in this section should be examined for possible",
    " errors or omissions in the larval survey data."
  )

  # table of input files
  tab <- cbind(
    Source[c("CatchFile", "LengthsFile", "PlotsFile")],
    c(
      "Current year catches", "Current year lengths",
  ...
  outfilenames <- c(
    catchout.file.name, lengthout.file.name, bpestout.file.name,
  ...
  if (Continue) {
    tab <- cbind(
      ...
      c(
        "Current year report, this document",
  ...
  if (dim(tab)[1] > 0) {
    tabl("These rows will be deleted prior to joining the ",
  ...
  if (dim(tab)[1] > 0) {
    tabl("Comment says too deep/shallow to sample, but habitat type and/or",
      ...
      " comment should be changed to 'No comment'.",
      row.names = FALSE, TAB = tab
    )
  }
  if (length(selboth) > 0) {
  ...
  tab <- with(smr2, smr2[
    is.na(hab.type) | is.na(region) | is.na(sl.total),
    printvars
  ])
  if (dim(tab)[1] > 0) {
  ...
  tab <- with(smr2, smr2[
    is.element(sampid, sampid[duplicated(sampid)]),
    printvars
  ])
  if (dim(tab)[1] > 0) {
    errors <- append(errors, paste("Table", GLFCenv$tabcount))
  ...
  if (dim(keep)[1] > 0) {
    for (i in 1:dim(keep)[1]) {
      ...
      dist.in.m <- geosphere::geodesic_inverse(
        smr2[
          keep[i, 1],
          c("longitude", "latitude")
        ],
        smr2[keep[i, 2], c("longitude", "latitude")]
      )[, "distance"]
      if (dist.in.m < 30) {
        ...
        tabl(
          paste0(
            "These two samples are very close together (",
            round(dist.in.m),
  ...
  if (dim(tab)[1] > 0) {
    errors <- append(errors, paste("Table", GLFCenv$tabcount))
    tabl("Sample is not listed as initial (sample=2).",
      row.names = FALSE,
      TAB = tab
    )
  }
  ...
  if ((ssc - nll) != 0) {
    para(
      "Number of lengths in LENGTHS file, ", nll,
  ...
  tab <- smr2[sel, c(
    "sampid", "date", "region", "new.numb", "depth",
  ...
  if (dim(tab)[1] > 0) {
    errors <- append(errors, paste("Table", GLFCenv$tabcount))
    tabl("Comment says Type 3, but habitat type disagrees.",
      row.names = FALSE,
      TAB = tab
    )
  }
  ...
  tab <- with(smr2, format(signif(tapply(
    sl.total, list(boat, inbplot),
    mean
  ), 2)))
  tabl("Mean (unadjusted) density (number per sample) of sea lampreys by",
    ...
    " may indicate a malfunction in one of the boats.",
    TAB = tab
  )
  ...
  fig <- function() {
    ...
    abline(h = 0)
    barplot(am2,
      ylim = c(0, maxfreq + 0.2), xlab = "", ylab = "",
      main = "Adjusted Catch"
    )
    abline(h = 0)
  ...
  fig <- function() {
    ...
    options(warn = -1)
    with(smr2, hist(date,
      breaks = max(as.numeric(date), na.rm = TRUE) -
  ...
  figu("Number of deepwater electrofisher samples taken over time during the ",
    YEAR, " larval sea lamprey survey.",
    FIG = fig, h = 3, w = 6.5
  )
  ...
  fig <- function() {
    ...
    with(smr2, hist(dec.time,
      breaks = ceiling(max(dec.time, na.rm = TRUE)) -
