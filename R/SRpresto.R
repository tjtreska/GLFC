...
#' }
SRpresto <- function(
    FOLDER, INDEX.LAKE, INDEX.STREAM, MAXLARVAE,
  ...
  heading(paste(
    "= = = = = = =   Status Report Tables and Figures -", TODAY,
    "  = = = = = = ="
  ))
  ...
  axisr <- read_excel(paste0(FOLDER, AXISRANGES),
    skip = 2, na = "NA",
    col_names = c(
      "m.order", "metric", "l.order", "lake",
  ...
  control <- data.frame(
    treat.year = year, lake = lake, staff.days = days,
  ...
  names(trout) <- c(
    "trout.year", "lake", "trout", "trout.lo", "trout.hi",
  ...
  rm(
    troutfiles, trouts, selcols, TROUTSUP, TROUTMIC, TROUTHUR, TROUTERI,
    TROUTONT
  )
  ...
  names(maxlarvae) <- make.names(casefold(names(maxlarvae)),
    unique = TRUE,
    allow_ = FALSE
  )
  ...
  top50maxlarvae$textpaste <- paste(
    top50maxlarvae$stream.nam,
  ...
  ALLlong <- ALL %>%
    ...
    pivot_longer(
      cols = c(index, rate, trout), names_to = "metric",
      values_to = "mean3"
    ) %>%
    filter(!is.na(mean3))
  ...
  bigfig1 <- function() {
    par(
      mfcol = c(3, 5), mar = c(3, 3, 0, 0), oma = c(2, 3, 2, 1), xaxs = "i", yaxs = "i",
      cex = 1
    )
    mycex <- par("cex")
    for (i in 1:5) {
      ...
      if (sum(!is.na(index[sel])) > 0) {
        plot(spawner.year[sel], index[sel] / 1000,
          type = "n", axes = FALSE,
          xlim = range(year.tk),
        ...
        points(spawner.year[sel], index[sel] / 1000,
          pch = 18,
          col = blindcolz[col.spa]
        )
        if (i == 1) {
          mtext("Adult sea lamprey\nindex  (thousands)",
            side = 2,
      ...
      if (sum(!is.na(rate[sel])) > 0) {
        plot(spawner.year[sel], rate[sel],
          type = "n", axes = FALSE,
      ...
      if (sum(!is.na(trout[sel])) > 0) {
        plot(spawner.year[sel], trout[sel],
          type = "n", axes = FALSE,
  ...
  bigfig2 <- function() {
    par(
      mfcol = c(3, 5), mar = c(3, 3, 0, 3), oma = c(2, 2, 2, 3), xaxs = "i", yaxs = "i",
      cex = 1
    )
    mycex <- par("cex")
    for (i in 1:5) {
      ...
      if (sum(!is.na(staff.days[sel])) > 0) {
        plot(spawner.year[sel], staff.days[sel] / 100,
          type = "n", axes = FALSE,
          xlab = "", ylab = "",
      ...
      if (sum(!is.na(tfmkgai[sel])) > 0) {
        plot(spawner.year[sel], tfmkgai[sel] / 1000,
          type = "n", axes = FALSE, xlab = "",
          ylab = "",
      ...
      if (sum(!is.na(bayerkgai[sel])) > 0) {
        plot(spawner.year[sel], bayerkgai[sel],
          type = "n", axes = FALSE, xlab = "",
          ylab = "",
  ...
  for (i in 1:5) {
    ...
    addPageBreak(this = doc, width = 8.5, height = 11, omi = c(1, 1, 1, 1))
    heading(paste(
      "STATUS OF SEA LAMPREY CONTROL IN LAKE",
    ...
    if (sum(!is.na(index[sel])) > 0) {
      ...
      targphrase <- paste0(
        "the mean of indices during a period with",
    ...
    fig <- function() {
      ...
      p1 <- ggplot(df, aes(long, lat)) +
        ...
        geom_point(aes(size = indexContrib, color = selbig),
          shape = 1, stroke = 2,
          na.rm = TRUE
        ) +
        scale_size_continuous(range = c(1, 10)) +
        scale_color_manual(values = c("FALSE" = unname(blindcolz)[2], "TRUE" = unname(blindcolz)[6])) +
        geom_label_repel(aes(label = strname),
          label.size = NA, segment.color = NA,
      ...
      p2 <- ggplot(df, aes(longitude, latitude)) +
        coord_quickmap() +
        geom_path(data = mapp, aes(x, y), color = "lightgray", na.rm = TRUE) +
        geom_point(aes(size = estimate),
          shape = 1, stroke = 1 + selbig,
          color = blindcolz[3], na.rm = TRUE
        ) +
        scale_size_continuous(range = c(1, 10)) +
        geom_label_repel(
          data = df[selbig, ], aes(label = stream.nam),
    ...
    if (sum(!is.na(rate[sel])) > 0) {
      fig <- function() {
        ...
        axis(1,
          at = tytickat, labels = tyticklab, tck = 0.015, cex.axis = 0.8,
    ...
    trout.fig.captions <- c(
      paste0(
        "Lake trout relative abundance (May assessments using 4.5 inch",
      ...
      paste0(
        "Lake trout relative abundance plotted against sea lamprey",
      ...
      paste0(
        "Lake trout relative abundance from standardized surveys (spring",
      ...
      paste0(
        "Lake trout relative abundance from standardized spring surveys",
    ...
    heading(
      "Lampricide Control - Adults vs. Field Days, TFM, and Bayluscide:",
      2
    )
    fig <- function() {
      par(
        mar = c(3, 3, 0.5, 5), oma = c(2, 2, 0, 0), mfrow = c(3, 1), xaxs = "i",
        yaxs = "i", cex = 1
      )
      mycex <- par("cex")
      if (sum(!is.na(staff.days[sel])) > 0) {
