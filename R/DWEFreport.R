...
DWEFreport <- function(
    Dir, CatchClean, LengthsClean, Plots, CatHist, LenHist,
  ...
  printvars <- c(
    "sampid", "date", "region", "new.numb", "depth", "hab.type",
    "sl.total", "commentwrap"
  )

  if (length(unique(CatchClean$year)) != 1) {
    stop(
      "There should be just one year",
      " of data in the Catch file."
    )
  }
  YEAR <- CatchClean$year[1]

  if (length(unique(CatchClean$period)) != 1) {
    stop(
      "There should be just one",
      " value for the variable period in the Catch file."
    )
  }
  WHEN <- GLFC::recode(CatchClean$period[1], c(0, 1, -1, NA), c(
    "No treatment",
  ...
  heading(paste0(
    "TITLE:  Draft Results of St. Marys River Larval Assessment ",
    YEAR
  ), 2)
  ...
  if (!is.null(Errors)) {
    heading(paste0(
      "These results are PRELIMINARY.  Unresolved errors remain",
  ...
  if (Downstream) {
    ...
    sigf <- 1
    para(
      "The larval sea lamprey survey in ", YEAR,
    ...
    if (nlower > 100) {
      ...
      cat(paste0(
        "Hmmm.  That's weird.  There were fewer than usual (", nlower,
    ...
    if (agef > 4) {
      para(
        "These expansion factors should probably be updated, since they",
    ...
    if (nlower < 1) {
      cat(
        "Okay.  We will have to apply an expansion factor to the survey data",
        " to get a whole river estimate of abundance.\n\n"
      )
    } else {
      cat(paste0(
        "Hmmm.  That's weird, because there were ", nlower,
        " samples taken south of latitude ", latcuttxt,
  ...
  if (sum(CatchClean2$latitude[CatchClean2$new.numb %in% treated.plots] <
    latcut) > 0 & uponly) {
    para(
      "Because some lower river hotspots were treated, but only the upper",
      " river was surveyed,",
      " we can only estimate abundance for the time period sampled."
    )
  }
  ...
  if (any(is.na(matchstrat))) {
    stop(paste0(
      "Some surveyed strata (",
  ...
  df.pre.est$sl.larv.adj[sel] <- with(
    df.pre.est[sel, ],
  ...
  df.post.est$sl.larv.adj[sel] <- with(
    df.post.est[sel, ],
  ...
  doit <- function(df) {
    ...
    strcount2 <- data.frame(
      stratum = names(strcount), n = as.numeric(strcount),
    ...
    out <- list(
      muhat.f = muhat.f, sigmahat.f = sigmahat.f,
  ...
  NewPE2 <- BlankPE %>%
    mutate(
      ...
      Period = 1,
      Trt = "post",
      Dates = paste(month(Date.Range), mday(Date.Range),
        sep = "/",
        collapse = " - "
      ),
      Samples = dim(smr3)[1],
  ...
  rm(
    CatchClean, areas3, nsamples.in.plots, surveyed.plots, surveyed.plots.area,
  ...
  # text
  para(
    WHEN, " sampling for larval sea lampreys was conducted from ",
  ...
  bkilltalk <- if (WHEN == "Post-treatment") {
    ""
  } else {
    paste0(
      "  For hotspots that were surveyed prior to treatment,",
  ...
  para(
    "A total of ", sum(smr3$sl.larv.n), " larval and ", sum(smr3$sl.meta.n),
  ...
  para(
    "Information from the survey was also used to generate hotspot-specific",
  ...
  region.names <- c(
    "North Channel", "Turning Basin", "Lake Nicolet",
  ...
  tab <- data.frame(
    regions, region.names[match(regions, regs)], nsamples[, 1],
    catch[, 1], dens[, 1], nsamples[, 2], catch[, 2], dens[, 2]
  )
  names(tab) <- c(
    " ", "Region", "Out.N", "Out.Catch", "Out.Dens", "In.N",
    "In.Catch", "In.Dens"
  )
  ...
  tab <- data.frame(matrix(NA, ncol = 8, nrow = 3, dimnames = list(
    NULL,
  ...
  tab$PE <- c(
    format(signif(pre.ests$PE / 1000000, 2)), " ",
    format(signif(post.ests$PE / 1000000, 2))
  )
  tab$SD <- c(
    format(signif(pre.ests$PE.sd / 1000000, 2)), " ",
    format(signif(post.ests$PE.sd / 1000000, 2))
  )
  tab$CV <- c(
    format(signif(pre.ests$PE.cv, 2)), " ",
    format(signif(post.ests$PE.cv, 2))
  )
  tab$CI.lo <- c(
    format(signif(pre.ests$PE.ci[1] / 1000000, 2)), " ",
    format(signif(post.ests$PE.ci[1] / 1000000, 2))
  )
  tab$CI.up <- c(
    format(signif(pre.ests$PE.ci[2] / 1000000, 2)), " ",
  ...
  tabl("Pre- and post-treatment population estimates of larval sea lampreys",
    ...
    " 95% confidence intervals (CI.lo and CI.up).",
    TAB = tab, row.names = FALSE
  )
  rm(area.treated)

  # plot-specific estimates
  tab <- pid3[pid3$larvpe > 0, c(
    "new.numb", "area.ha", "trtd", "n.samp", "catch",
    "meannperha", "sd.dens", "larvpe", "pbig", "bigpe", "bigdens"
  )]
  tab$hotspot <- tab$new.numb
  tab$cum.area <- cumsum(tab$area.ha)
  tab <- tab[, c(
    "hotspot", "area.ha", "cum.area", "trtd", "n.samp", "catch",
    "meannperha", "sd.dens", "larvpe", "pbig", "bigpe", "bigdens"
  )]
  ...
  tabl("Post-treatment ", YEAR, " population estimates of larval sea lampreys",
    ...
    "  Only those hotspots with positive catch are shown.",
    TAB = tab
  )
  ...
  fig <- function() {
    par(yaxs = "i", xaxs = "i")
    maps::map(
      type = "n", xlim = range(smr3$longitude, na.rm = TRUE) + c(-0.002, 0.002),
    ...
    # treated plots
    points(smr3$longitude[sel.trt], smr3$latitude[sel.trt],
      pch = 16, cex = 1,
      col = "yellow"
    )
    # river boundary
    ...
    points(smr3$longitude[!sel.hit], smr3$latitude[!sel.hit],
      pch = 16,
      col = "blue", cex = 0.4
    )
    points(smr3$longitude[sel.hit], smr3$latitude[sel.hit],
  ...
  figu("Design and catch of the ", YEAR,
    " larval sea lamprey survey in the St. Marys River.",
    FIG = fig,
    newpage = "port"
  )
  rm(sel.trt, sel.hit)
  ...
  am2 <- matrix(NA,
    nrow = dim(lens.plus)[1], ncol = length(all5),
    dimnames = list(NULL, all5)
  )
  am <- tapply(
    lens.plus$big, list(seq(dim(lens.plus)[1]), lens.plus$len5),
    sum
  )
  ...
  fig <- function() {
    ...
    barplot(am2 / 1000,
      ylim = c(0, maxfreq + 0.2) / 1000, xlab = "Length  (mm)", ylab = "",
      las = 1
    )
    abline(h = 0)
  ...
  fig <- function() {
    par(yaxs = "i", xaxs = "i")
    maps::map(
      type = "n", xlim = range(pid3$meanlong, na.rm = TRUE) + c(-0.002, 0.002),
      ylim = range(pid3$meanlat, na.rm = TRUE), mar = c(0, 0, 0, 0)
    )
    # river boundary
    addsmr(col = "lightgray")
    points(pid3$meanlong[!sel.z], pid3$meanlat[!sel.z],
      pch = 21,
  ...
  figu("Estimated ", YEAR, " post-treatment larval sea lamprey density in",
    ...
    " Xs represents hotspots with zero catches.",
    FIG = fig
  )
  rm(
    post.in, post.in.dens, pid2, EOGS, ADG, b0, b1, lens2, probs, dens.cat,
    colz, sel.trt
  )
  ...
  newlens <- LengthsClean[
    LengthsClean$class == "A",
  ...
  newplots <- pid3[, c(
    "year", "period", "new.numb", "meanlat", "meanlong",
